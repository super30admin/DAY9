...
/*
 * By storing each element upto the max along with their respective values
 * we can convert this to a house-robber pattern
 * We can use a map to store the pairs, or an array - an array would
 * be faster and simpler to code, but would need upto maxElem space
 * A map would be slightly slower and need a condition for when the element
 * is not present but would me more space efficient, especially if the
...
class DeleteAndEarn {
    public int deleteAndEarn(int[] nums) {
        ...
        // for each possible element upto max
        for(int i = 1; i <= max; i++) {
